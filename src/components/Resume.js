import React, { Component, useState } from "react";
export default function Resume(props) {
  let resumeData = props.resumeData;

  const [readout, setReadout] = useState("");

  return (
    <section id="resume">
      <div className="centered">
        <div
          className="row centered"
          style={{
            margin: "0 auto",
            position: "relative",
            // right: "5vw",
            width: "100vw",
          }}
        >
          <div className="three columns header-col">
            <h1>
              <span>SKILLS</span>
            </h1>
          </div>
          <div
            className="nine columns main-col centered"
            style={{
              // width: "80vw",
              // paddingLeft: "10vw",
              // paddingRight: "10vw",
              // height: "100vh",
              justifyContent: "space-between",
              margin: "0 auto",
              display: "flex",
              // left: "50%",
              // right: "50%",
              alignContent: "center",
              position: "relative",
            }}
          >
            <div className="skillColumn">
              <div
                className="skills row"
                style={{
                  flexDirection: "row",
                  justifyContent: "space-between",
                }}
              >
                <div className="column">
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("React")}
                      onMouseOut={() => setReadout("")}
                      id="react"
                      className="pin"
                      width="150px"
                      height="auto"
                      src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/1200px-React-icon.svg.png"
                    />
                    <p className="centerText">React</p>
                  </li>
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("Node")}
                      onMouseOut={() => setReadout("")}
                      id="node"
                      className="pin"
                      width="150px"
                      height="auto"
                      src="https://nodejs.org/static/images/logo-hexagon-card.png"
                    />
                    <p className="centerText">Node</p>
                  </li>
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("Node")}
                      onMouseOut={() => setReadout("")}
                      id="node"
                      className="pin"
                      width="150px"
                      height="auto"
                      src="https://youteam.io/blog/wp-content/uploads/2022/06/django-icon-0.png"
                    />
                    <p className="centerText">Django</p>
                  </li>
                </div>
                <div className="column">
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("AWS")}
                      onMouseOut={() => setReadout("")}
                      className="pin"
                      id="aws"
                      width="150px"
                      height="auto"
                      src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/93/Amazon_Web_Services_Logo.svg/512px-Amazon_Web_Services_Logo.svg.png"
                    />
                    <p className="centerText">AWS</p>
                  </li>

                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("Git")}
                      onMouseOut={() => setReadout("")}
                      className="pin"
                      id="git"
                      alt=""
                      width="140px"
                      height="auto"
                      src="https://git-scm.com/images/logos/downloads/Git-Icon-Black.png"
                    />
                    <p className="centerText">Git</p>
                  </li>

                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("Git")}
                      onMouseOut={() => setReadout("")}
                      className="pin"
                      id="git"
                      alt=""
                      width="140px"
                      height="auto"
                      src="https://logodix.com/logo/1614302.png"
                    />
                    <p className="centerText">Spring Boot</p>
                  </li>
                </div>
                <div className="column" style={{ float: "left" }}>
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("Java")}
                      onMouseOut={() => setReadout("")}
                      id="java"
                      className="pin"
                      // width="80px"
                      height="auto"
                      src="data:image/png;base64,"
                    />
                    <p className="centerText">Java</p>
                  </li>
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("PostgreSQL")}
                      onMouseOut={() => setReadout("")}
                      className="pin"
                      id="postgres"
                      alt=""
                      // width="100px"
                      // height="auto"
                      src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/Postgresql_elephant.svg/1200px-Postgresql_elephant.svg.png"
                    />
                    <p className="centerText">PostgreSQL</p>
                  </li>
                  <li
                    className="glowText"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      justifyContent: "space-between",
                    }}
                  >
                    <img
                      onMouseOver={() => setReadout("PostgreSQL")}
                      onMouseOut={() => setReadout("")}
                      className="pin"
                      id="postgres"
                      alt=""
                      // width="100px"
                      // height="auto"
                      src="https://upload.wikimedia.org/wikipedia/commons/e/e9/Jenkins_logo.svg"
                    />
                    <p className="centerText">Jenkins</p>
                  </li>
                </div>
                {/* <div className="readoutContainer">
                  <p className="centered readout">{readout}</p>
                </div> */}
              </div>
              <div className="education"></div>
              <div className="nine columns main-col col centered">
                <img
                  className="pin"
                  id="postgres"
                  alt=""
                  width="100px"
                  height="auto"
                  src="https://scontent-atl3-2.xx.fbcdn.net/v/t1.6435-9/80111037_10157563266401352_8641762864303964160_n.jpg?_nc_cat=111&ccb=1-7&_nc_sid=09cbfe&_nc_ohc=DwkQ6a-ck5MAX9et0ef&_nc_ht=scontent-atl3-2.xx&oh=00_AfBxICwkcpMy0P-tlBhziM0c9kHrNMlsJX6kBMtyH7HrRQ&oe=63B97D22"
                />
                <div style={{ margin: "2em" }}>
                  {resumeData.education &&
                    resumeData.education.map((item) => {
                      return (
                        <div className="row item">
                          <div className="twelve columns">
                            <h3>{item.UniversityName}</h3>
                            <p className="info">
                              {item.specialization}
                              <span>&bull;</span>{" "}
                              <em className="date">
                                {item.MonthOfPassing} {item.YearOfPassing}
                              </em>
                            </p>
                            <p>{item.Achievements}</p>
                          </div>
                        </div>
                      );
                    })}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
